  ...
  return (
    <section className="bg-white py-8 border-b border-ira-border">
      <div className="w-[calc(100%-56px)] mx-auto">
        <div className="grid max-md:grid-cols-6 max-md:gap-x-2 max-md:gap-y-8 md:grid-cols-5 md:gap-4 text-center">
          {highlights.map((item, i) => (
            <Reveal 
              key={i} 
              delay={i * 100} 
              className={`max-md:col-span-2 ${i === 3 ? "max-md:col-start-2" : ""}`}
            >
              <div className="flex flex-col items-center justify-center">
                {item.icon}
                <h3 className="text-[11px] max-md:text-[10px] max-[390px]:text-[9px] uppercase tracking-[0.1em] text-ira-teal font-medium m-0 max-w-[120px] mx-auto">
                  {item.title}
                </h3>
              </div>
            </Reveal>
